...
func TestParse(t *testing.T) {
	...
	for _, tt := range ts {
		pf := &jellyfinFile{name: tt.n, mov: movie{}}
		pf.parse()
		if pf.mov.name != tt.m {
...
func TestJellyfinName(t *testing.T) {
	...
	for _, tt := range ts {
		pf := &jellyfinFile{mov: movie{name: tt.m, year: tt.y, season: tt.s, episode: tt.e, epiName: tt.en}}
		pn := pf.jellyfinName()
		if pn != tt.n {
			t.Errorf("got:  %s\nwant: %s", pn, tt.n)
		}
...
func TestJellyfinDir(t *testing.T) {
	ts := []struct {
		m, y, s, e, en string
		d              string
	}{
